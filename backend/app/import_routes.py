...
their apply step runs as a background job instead of writing inline (D-166) and additionally support saved
...
def _require_access(auth, entity):
    ...
    `university_contacts`/`interactions` are open to any CRM role, scoped per-row (D-167)."""
...
def apply_import(import_id: int, data: MappingIn, request: Request, auth: AuthContext = Depends(any_role), db: Session = Depends(get_db)):
    ...
    # Idempotency (D-166): a queued/running apply job for this import already covers this request.
