...
§3.3/3.4; decisions D-155, D-158, D-159, D-160).

A `Document` is a versioned, titled file library entry polymorphically linked to a `university`, `launch`
(interaction) or `contract` -- deliberately separate from the workflow `attachments` table (D-159). Each
...
DOWNLOAD_LINK_TTL_SECONDS = 300  # 5 minutes (D-160)
...
def admin_access_payload(user, base):
    # D-158: every administrator read of document content is flagged separately in the audit payload so a
...
def download_document_version(
    ...
    if not verify_download_token(settings, version_id, token, expires):
        ...
        raise AppError(ErrorCode.RECORD_NOT_FOUND)
    # D-160: the token proves the link was legitimately issued, not that access is still valid -- scope is
