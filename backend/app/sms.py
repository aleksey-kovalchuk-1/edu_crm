"""Injectable SMS sending for CRM-owned phone verification (D-156, D-157).

No real SMS gateway is integrated in this environment (no credentials or provider API spec were
available — see docs/decisions.md D-157). `send_sms` picks one of two concrete implementations
...
- Set: a generic, best-effort HTTP POST to `sms_provider_url`:
  ...
  gateway (presumably Rostelecom-operated, per D-157) was not available to integrate against or
