...
| T-033 | xls/xlsx import: upload, column mapping (saved profiles), validation preview, idempotent apply, import report | T-031 | A synthetic sample file imports; invalid rows reported with row number and reason; re-import creates no duplicates | VERIFIED except signed-in browser check (`64c2642` API, `43115a0` wizard; synthetic `docs/samples/catalog-import-sample.xlsx` reports 6 valid rows and row 9 invalid date; re-apply returns 409; 98 frontend tests; deployed; signed-in check needs the owner per D-130). **Phase 6 re-verification after integration** (unchanged by Phases 1-5, no rebuild needed): the manual live check above is now `tests/test_import_api.py::test_the_documented_sample_workbook_imports_as_recorded_in_the_night_report` (same 6 valid / row 9 invalid / 409-on-repeat facts, plus Cyrillic university names surviving the full upload→check→apply→database round trip); a new `test_import_scope_and_permissions_cannot_be_bypassed_through_direct_api_calls` confirms a `crm-user` gets 403 from every import endpoint called directly, not just from the wizard UI; legacy `.xls` and file-signature-vs-extension checks (`test_legacy_xls_file_can_be_imported`, `test_non_excel_uploads_are_rejected`) already existed and still pass. 306 backend tests total, no regressions found |
...
| ID | Task | Depends on | Acceptance criteria | Status |
|---|---|---|---|---|
| T-070 | Security hardening: headers and CSP, login rate limiting, audit immutability, dependency audit, threat model and measures mapping (no compliance claims) | T-023 | Checks recorded; document maps each measure to code | PARTIAL for the integration gate specifically (headers/CSP/login rate limiting predate this integration and were unaffected -- see M2's own T-020/security-review entries): `docs/operations/security.md` maps auth/RBAC/scope/CSRF/audit-immutability/data-minimisation/storage-separation/no-external-AI to the code that implements each, and states what's not done (real connector/reCAPTCHA/SMTP secrets, TLS, closed-network isolation, admin break-glass, no 152-FZ/FSTEC claim); `pip-audit -r backend/requirements.txt` and `npm audit --omit=dev` both report no known vulnerabilities at this commit (ad hoc, not wired into CI -- none was configured before) |
| T-071 | Load test: 50 concurrent users and 10 parallel reports | T-054 | Stored results with percentiles; no claim beyond measured numbers | PARTIAL: only the 10-parallel-report half was in this integration's scope, and is VERIFIED (`tests/test_reports.py::test_ten_parallel_report_jobs_complete` -- 10 report_generate jobs processed across 10 threads via `SELECT...FOR UPDATE SKIP LOCKED`, all 10 succeed with one file each). The 50-concurrent-user half (with stored percentile results) was not attempted and stays TODO |
