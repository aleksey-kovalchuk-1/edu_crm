...
Сценарий и правила разбора договоров — `docs/design/import.md`; обобщённый импорт по сущностям и фоновая модель — `docs/design/file-ingestion-plan.md` §3.1/§3.2, решения D-156, D-157, D-162 в `docs/decisions.md`. Изменяющие запросы требуют `X-CSRF-Token`. Ошибки — `{code, message, details}`.

## Сущности

Каждый запрос указывает `entity`: `universities`, `university_contacts`, `interactions` (только создание — обновление статуса взаимодействия через импорт не предусмотрено, D-157) или `contracts` (исходный поток T-033, поведение не изменилось). Без параметра — по умолчанию `contracts`, для обратной совместимости.
