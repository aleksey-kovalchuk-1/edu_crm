...
| I-008 | Toggling a task also refetched `/launches` in the browser check | Claude Browser network log on 2026-09-15: `PATCH /tasks/1` followed by `GET /tasks` and `GET /launches`, while the mutation only invalidates tasks; TanStack Query refetches stale active queries on window focus with the default `staleTime: 0` | Fewer requests per interaction; supports the 1-second requirement | Small; query client defaults (e.g. `staleTime` of 30 s) | Network log shows only the invalidated query after a toggle; frontend tests still pass | CANDIDATE |
| I-009 | Login state rows for abandoned logins accumulate between logins | `login_states` rows are deleted only when the next login starts | Keeps the table small | Small | Periodic cleanup or a test that expired rows are removed | CANDIDATE |
