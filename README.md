...
Опциональная переменная `INITIAL_SUPERADMIN_EMAIL` (по умолчанию не задана) при каждом старте контейнера `api` проверяется скриптом `python -m app.bootstrap_superadmin`: если в Keycloak ещё нет ни одного пользователя с ролью `crm-superadmin`, роль один раз выдаётся пользователю с этим email (если такой найдётся); при повторных запусках и при уже существующем суперадмине ничего не делает.
